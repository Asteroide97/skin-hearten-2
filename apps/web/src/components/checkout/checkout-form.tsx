...
export function CheckoutForm() {
  ...
  async function handleCheckoutSubmit(values: CheckoutValues) {
    if (items.length === 0) {
      setSubmitError("Tu rutina esta vacia. Anade productos antes de confirmar el pedido.");
      return;
    }
  ...
  return (
    <div className="space-y-8">
      <SkinRoutineBanner context="checkout" />
      ...
      <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_360px]">
        ...
        >
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.25em] text-stone-500">Envio protegido</p>
            <h2 className="mt-2 font-serif text-3xl text-stone-900">Tu rutina casi esta lista</h2>
          ...
          <fieldset className="space-y-3">
            <legend className="text-sm font-semibold text-stone-900">Pago seguro</legend>
          ...
          <button
            ...
            type="submit"
          >
            {isSubmitting ? "Preparando tu rutina..." : "Confirmar mi rutina"}
          </button>
        </form>
        ...
        <aside className="soft-panel h-fit rounded-[1.8rem] p-6">
          ...
          <div className="mt-6 space-y-4 text-sm text-stone-700">
            {items.map((item) => (
              <div className="flex items-start justify-between gap-3" key={item.productId}>
            ...
            <div className="flex items-start justify-between gap-3 border-t border-stone-200 pt-4">
              <span>Subtotal</span>
            ...
            <div className="flex items-start justify-between gap-3 text-base font-semibold text-stone-900">
              <span>Total</span>
          ...
          <div className="mt-8 space-y-3 rounded-[1.6rem] bg-white/80 px-4 py-4 text-sm leading-7 text-stone-600">
            ...
            <p>No hacemos pruebas en animales.</p>
            <p>Envio protegido.</p>
          </div>
        </aside>
      </div>
